...
def cori_plot_two_avg_irrads(study_name, base_dir, grid1, grid2, log_data=True, log_err=True):
    ...
    ax1.set_xlabel('z')
    ax1.set_ylabel('Perceived Irradiance')
    ax1.legend()
    if log_data:
        ax1.set_yscale('log')
...
def cori_plot_two_avg_irrads_onespace(study_name, grid1, grid2, *args, **kwargs):
    grid1['nz'] = grid1['ns']
    grid2['nz'] = grid2['ns']
    cori_plot_two_avg_irrads(study_name, grid1, grid2, *args, **kwargs)
...
def grid_study_analyze_full(db_path, table_name, base_dir):
    ...
    best_perceived_irrad = calculate_perceived_irrad(p_kelp, best_irrad)
